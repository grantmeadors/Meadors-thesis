...
Denis Martynov, Anamaria Effler, Robert Ward, and Ryan deRosa commented on the lock. MICH noise was dominating the spectrum. Based on Keita Kawabe, Keith Riles and my subtraction of MICH \& PRC noise from the S6 data set (and H1 in real-time), I wrote a Matlab script to investigate.  
...
using Gregory Mendell's \texttt{readFrames} function to pull 2100 s of frame data starting 1085202300.

Absent a DARM\_ERR or \_CTRL channel, the \_IN1\_DQ channels were used, following the example of Rana Adhikari in the LLO aLog. Kawabe subsequently confirmed that IN1\_DQ has replaced ERR and OUT\_DQ has replaced CTRL.
...
These results corroborate Martynov, Effler, Ward and deRosa's assessment that MICH noise is currently dominant and suggest that PRCL and SRCL noise are also significant. To be thorough this result should be verified using the OUT\_DQ channels; these results were calculated using IN1\_DQ.
