...
Both polarization states, $h_\times$ \& $h_+$, can be measured directly.
...
In coordinate system of Equation~\ref{GW-matrix-eq}, this would be a plus-polarized $h_+$ wave travelling along the $z$-axis, with arms aligned on $x$- and $y$-axes.
...
Given our expectations for GWs are predicted by GR, LIGO and allies have built highly-sensitive interferometers to search for the signals anticipated in Section~\ref{estimates}.
...
Figure~\ref{newNOISEfilter} illustrates the closed loop gain.
...
If pursued, more sophisticated angular controls might be available for future interferometers.
...
Fused silica fibers with high mechanical $Q$ suspend the mirrors, instead of steel piano wire; fibers are welded directly to the test mass.
...
In Japan, KAGRA~\cite{Kuroda2010} is being built; when completed with sapphire mirrors and cryogenic systems, it should reach comparable sensitivity to Advanced LIGO interferometers.
...
Around the same time as these observatories make first detection, a different technique of GW astronomy, pulsar timing~\cite{Hobbs2010}, may open up vastly lower frequencies ($\mathcal{O}(10^{-9})$ Hz).
Pulsar timing uses radio telescopes to measure the arrival time of pulses, with characteristic wavelengths of order of one light-year.
...
If eLISA launches, it may open the way for measurements of the most elusive gravitational signature: the background of the universe itself, analogous to the cosmic microwave background.
