...
Several approaches exist toward a search from GW from known objects such as Scorpius X-1\footnote{Discovered by Riccardo Giacconi.}.
...
The 60 Hz and first three harmonic lines (120, 180, 240) Hz harmonic lines, as well as frequencies new the violin modes around 330 Hz, are excluded by these tests.
...
TwoSpect's S6 UL is competitive (possibly better, pending deeper understanding of the algorithmic differences), particularly given that S6 has half the data of S5.
Moreover, given an outlier, TwoSpect could do something not yet possible with other comparable pipeline: identify the projected semimajor axis, $a \sin i$.
...
Some coincident outliers present in the initial analysis include lines at powers of 2 and a probable trace of a hardware-injected pulsar.
...
Discovered by Markwardt in 2002~\cite{Markwardt2002}, the X-ray transient (XTE) J1751-305 has indications it too could emit GW~\cite{Strohmayer2014}.
...
Although no $r$-modes have so far been detected, J1751 is an intriguing candidate in its own right and highlights the different kinds of directed GW search that TwoSpect can conduct.

J1751 is the XTE with the shortest known period,
P $\approx$$ $ 2545.3 seconds or 42 minutes, and $a \sin i$ $\approx0.010\pm0.003$s.
Crucially, its spin frequency, unlike Scorpius X-1, is known with microHertz precision: $f = 435.31799$ Hz.
A search can thus be fast ($< 10^5$ templates).
Although an estimated $d > 7$ kpc, near galactic center, the chance of a $r$-mode motivates a quick look.
...
This primary analysis (4 kHz of data between the two detectors) has yielded a joint upper limit over the 2 kHz from 40 to 2040 Hz.
...
In the process, our post-processing methodology has been adjusted from all-sky techniques~\cite{GoetzTwoSpectResults2011} to ones appropriate for a densely-sampled, correlated `$df$ vs $f$' parameter space.
The remaining questions are in the identification of outliers -- already $\sim 22$ coincident features have been noted between 40 and 360 Hz -- and explanation of quiet bands.
Quiet bands, seen in Figure~\ref{S6_H1L1_UL}, appear due to the noise subtraction in the $R$ statistic;further investigation will see whether these presumably non-genuine `good' bands can be systematically rectified.
